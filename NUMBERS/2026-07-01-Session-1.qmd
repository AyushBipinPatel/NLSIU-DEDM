...
Definitions -> Measurements -> organized format
...
## The usual guess: Mean and subgroups

<br><br>

[Data doesn't always consist units of observation form the same group.]{.fragment fragment-index='1'}
<br><br>
[Keep the `Penguins` data in mind. There are three different species of penguins in the data.]{.fragment fragment-index='2'}
<br><br>
[It is important to understand how subgroup means relate to mean of the sample/overall mean.]{.fragment fragment-index='3'}

## The usual guess: Mean and subgroups


The mean of all penguins is `r penguins |> filter_out(is.na(species) | is.na(body_mass_g)) |>summarise(a = mean(body_mass_g, na.rm =T))|> pull(a)`, but..

```{r}
penguins |>
  filter_out(is.na(species) | is.na(body_mass_g)) |>
  ...
  gtsummary::tbl_summary(
    ...
    missing = "no",
    digits = list(body_mass_g = c(3))
  )
```

Notice that $\frac{(3700.66 + 3733.09 + 5076.02)}{3} \ne 4201.754$

<br>

but $\frac{((3700.66*151) + (3733.09*68) + (5076.02*123))}{(151+68+123)} = 4201.754$

<br>

so, $Overall\space Mean = \frac{((N_{g1} * \bar{y_{g1}}) + (N_{g2} * \bar{y_{g2}}) + (N_{g3} * \bar{y_{g3}}))}{(N_{g1} + N_{g2} + N_{g3})}$
...
**Steps to calculating IQR**: Odd Number of Observations
...
5. *Identify Mid point in both halves*: IQR is $15-5 = 10$
...
**Steps to calculating IQR**: Even Number of Observations
...
5. *Identify Mid point in both halves*: IQR is $16-5$ = 11
...
# Comparing dispersion: Within and Across Distributions

## The Standard Deviation

<br>
[`sd` gives us an average distance of observations from the mean]{.fragment fragment-index='1'}
<br><br>
[unlike `variance`, it is in the same unit scale of the measure/variable]{.fragment fragment-index='2'}
<br><br>
[**But it fails to provide any useful information about dispersion of observations when compared within a distribution or on comparison of dispersion of two different distributions.**]{.fragment fragment-index='3'}
...
> Say, that all monarch butterflies come from a normal distribution of mean 680mg and sd of 32mg. A particular monarch butter fly weighs 744mg, named ButterKing. Also, imagine that all asiatic male elephants weights come from a normal distribution of mean 3600Kg and sd of 600Kg. A particular elephant weighs 4880 Kg, named Champak. **Who is more unusual when it comes to weights, ButtterKing or Champak?** 
...
> There are two distributions, one with mean 76 and sd 6.4, second with mean 176 and sd 16. which of the two distribution has more spread?

<br>
*doesnt work well with distributions with mean zero, close to zero or negative mean*

# Exercise for Excel {.smaller}

Get the data (`hh_98`) on from [here](https://microdata.worldbank.org/catalog/436/get-microdata),choose the csv files from micro data section. Use the documentaiton. Your goal is to generate two summary tables that convey the following information. Table structure can vary, but it should have all required information.
...
| - | Household land |  Household Assets |Per capita expediture on Food | 
|---|----------------|-------------------|------------------------------|
|Mean|  |  |  |
|Median|  |  |  |
|Standard deviation|  |  |  |
|IQR |  |  |  |
|34th percentile|  |  |  |
|Number of observarions withi +/- 1sd|  |  |  |
...
| - | Household land |  Household Assets |Per capita expediture on Food | 
|---|----------------|-------------------|------------------------------|
| Mean for HH with no participant |  |  |  |
| Mean for HH with only male participant |  |  |  |
| Mean for HH with only Female participant |  |  |  |
| Mean for HH with male and Female participants|  |  |  |
